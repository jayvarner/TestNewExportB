function initViewer(canvas, height, width, ocr, annotations) {
  ...
  fetch(`/overlays/ocr/${ocr}`)
          ...
          data.resources.forEach(word => {
            ...
            new OpenSeadragon.MouseTracker({
              ...
              overHandler: function(event) {
                const links = document.querySelectorAll(`[data-id="${event.originalEvent.target.getAttribute('data-id')}"]`);
                if (links) {
                  const annotationCard = document.querySelector(`[data-annotation-id="${event.originalEvent.target.getAttribute('data-id')}"]`);
                  if (annotationCard) {
              ...
              leaveHandler: function(event) {
                if (!event.originalEvent.target.firstElementChild) return;
                const links = document.querySelectorAll(`[data-id="${event.originalEvent.target.firstElementChild.getAttribute('data-id')}"]`);
                if (links) {
                  const annotationCard = document.querySelector(`[data-annotation-id="${event.originalEvent.target.firstElementChild.getAttribute('data-id')}"]`);
                  if (annotationCard) {
    ...
    fetch(`/overlays/annotations/${annotations}`)
    .then(
      function (response) {
        response.json().then(function (data) {
          ...
          svgContainer.node().style.pointerEvents = 'auto';
